...
const USAGE = "assemble-deck.mjs --job <job-id> --revision <working|revision-NNNNNN>";
...
async function main() {
  ...
  const html = await renderer.assemblePreview(options);
  const artifact = await renderer.writeQaArtifact({ ...options, filename: "preview.html", data: html });
  return { ok: true, ...options, artifactId: artifact.artifactId, byteLength: Buffer.byteLength(html, "utf8") };
}
