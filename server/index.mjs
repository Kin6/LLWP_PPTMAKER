...
function buildIntegratedTextImagePrompt(job, guide, referenceInstruction) {
  ...
  return `画一个完整的 16:9 高端 PowerPoint 成品页面，用于第 ${job.pageNumber} 页。第一眼必须明确看出这是经过专业信息设计的演示页，而不是电影海报、广告 KV、游戏宣传图、摄影作品或一张铺满文字的插画。页面需要有稳定的演示文稿网格、清楚页边距、标题区、证据区、辅助说明与页码系统；文字与视觉互相解释，但可读性和论证层级优先于戏剧化效果。\n\n【整套叙事位置】\n${buildSequenceContext(job)}\n\n【页面文案，必须逐字呈现，不得改写、翻译或虚构】\n演示名称：${job.deckTitle || "（不显示）"}\n主标题：${job.title}\n副标题：${job.subtitle || "（无）"}\n核心句：${job.claim || "（无）"}\n短要点：\n${bullets}\n数据标注：${callouts}\n表格数据：\n${table}\n页码：${page}\n\n【内容与构图任务】\n${job.prompt}\n页面原型：${archetype}\n布局线索：${job.layout || "visual-right"}。\n${referenceInstruction}\n整体审美方向：${guide.direction}。\n\n【图文融合规则】\n1. 使用真正的 PPT 信息层级：主标题占页面高度约 10% 到 16%，最多两行，不得做成占满三分之一画布的海报巨字。标题、核心句、最多两个短要点和页码必须各自拥有清楚位置，阅读顺序为“结论标题 -> 视觉证据 -> 解释或行动”。\n2. 页面四周保留稳定、可感知的留白或低细节边界。除封面外，主视觉通常占页面约 45% 到 65%，不要让摄影或 3D 场景无差别铺满每一页。允许使用克制的左右布局、上下布局、中心证据布局或一图一结论布局，但禁止卡片墙、仪表盘外壳和密集 HUD。\n3. 每页只保留一个与观点直接相关的主视觉和最多一种辅助证据形态，例如一张照片加两处标注、一幅机制示意加一个关键数字、一个对比画面加一句结论。不要为了丰富而堆叠图标、光圈、箭头、装饰线和无关模块。\n4. 文字应与视觉发生有节制的关系：可以沿主体轮廓、视线或因果轴对齐，也可以在明确的低细节区域内排版；不要用大面积遮挡、极端透视或多层半透明字破坏可读性。整套页面保持同一字体家族、色彩系统、图标线宽、页码位置、页边距和视觉母题，构图随论证任务变化，但骨架必须像同一套 PPT。\n5. 只使用上面提供的文字。没有提供的品牌、统计数字、按钮、网址、免责声明和伪界面文案一律不要生成。禁止从风格参考图中抄写任何文字。除非本页文案明确包含，否则严禁出现 DeckSpec、Image 2、API、Prompt、工作流、生成 PPT、视觉拆解、页面组装等制作过程内容。若文字过多，优先完整呈现主标题、核心句、页码和最短的两个要点，不得自行缩写或编造。\n6. 中文使用清晰、粗壮、现代的简体中文字体效果，笔画完整；数字和英文保持准确。小字号也要高对比，不要出现乱码、随机字符、[object Object] 或无意义占位文字。\n7. 风格参考图只用于学习色彩、材质、字体气质和图文层级，不要复制其中的题材、构图和巨型标题比例。优先呈现“结论 + 证据 + 含义”的演示逻辑，不要做成品牌海报或宣传封面。\n8. 整张图就是幻灯片本身，不要画投影幕、电脑屏幕、PPT 编辑器边框或页面外环境。所有文字、人物头部、机械主体、图表、图标和关键标注必须完整位于 x=7% 到 93%、y=10% 到 88% 的安全区内；任何字形、页码或主体都不得接触或越过画布边缘。最上方 10% 和最下方 12% 只允许连续背景或可安全延展的纹理，严禁放标题和底部信息带。`;
}
...
function buildIntegratedArtDirection(job) {
  ...
  return [
    ...
    "先建立清楚的演示页网格和四周安全边距，再安排标题、视觉证据和解释。允许克制的左右分区、上下分区或中心证据构图，但禁止卡片墙、仪表盘外壳和常规 SaaS 界面拼贴。",
    ...
    "整套页面共享同一色彩、字体气质、页边距、标题基线、图标线宽、页码位置和连续视觉母题；把所有页缩略图并排时必须显然属于同一套 PPT，同时每页只承担当前论证任务。",
  ].join("\n");
}
...
function imagePageArchetype(layout, pageNumber) {
  if (layout === "cover" || pageNumber === 1) return "演示封面：标题最多两行，稳定落在安全区内；一个英雄主体或关键场景建立主题，副标题与价值句简洁落位，保留明显页边距，不做海报巨字。";
  ...
  if (layout === "visual-right") return "右侧证据页：标题与核心句在左侧形成清楚层级，右侧主视觉负责证明；只保留一条主阅读路径和稳定页边距。";
