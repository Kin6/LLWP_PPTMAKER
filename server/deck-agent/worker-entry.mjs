        ...
        { role: "system", content: skill.instructions },
        {
          ...
          content: JSON.stringify({
            ...
            reviewInstruction: priorReport
              ? "The visible outline is the authoritative visible-content contract and intentionally excludes speaker notes. Ignore any conflicting presenter cue in the design brief; do not fail a slide for omitting note-only content. Recheck the supplied target slides against the previous blockers. Report only blockers that remain or objective regressions introduced by the repair. Judge layout safety, legibility, topic fit, visual hierarchy, full-canvas balance, and consistency. Intentional whitespace is acceptable when the composition remains balanced. Ignore player chrome such as navigation arrows, progress bars, and slide numbers. Text-free background decoration may intentionally bleed beyond the 72px content inset when it is cleanly clipped by the slide; apply the safe inset to readable content and meaningful media, not decorative background geometry."
              : "The visible outline is the authoritative visible-content contract and intentionally excludes speaker notes. Ignore any conflicting presenter cue in the design brief; do not fail a slide for omitting note-only content. Judge layout safety, legibility, topic fit, visual hierarchy, full-canvas balance, and cross-slide consistency. Report only concrete actionable failures; intentional whitespace is acceptable when the composition remains balanced. Ignore player chrome such as navigation arrows, progress bars, and slide numbers. Text-free background decoration may intentionally bleed beyond the 72px content inset when it is cleanly clipped by the slide; apply the safe inset to readable content and meaningful media, not decorative background geometry.",
          }),
        },
      ],
      schema: VISUAL_REVIEW_SCHEMA,
